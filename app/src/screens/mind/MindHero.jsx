...
export default function MindHero() {
  const { sleepMins, lastNight, mindDone, mindMood, setPillarInfo, healthSync, healthSource, setLogSleepOpen } =
    useWF();
  ...
  // Only the person entering their own nights can add one. Connected, the
  // number is a reading and a button on it would be a lie.
  const ownSleep = healthSource.sleep === "manual";
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <Ring
          as={ownSleep && !syncing ? "button" : "div"}
          onClick={ownSleep && !syncing ? () => setLogSleepOpen(true) : undefined}
          pct={pct}
        >
          {syncing ? (
            <Skel w={44} h={16} />
            ...
            <>
              <Moon size={17} color={MIND_C} strokeWidth={2} />
              <span style={{ fontSize: 11, fontWeight: 700, color: MIND_C, marginTop: 3 }}>Add</span>
            </>
          ) : (
...
/* The night, as a dial. A button when the person owns the number, a plain
   circle when their phone does. */
function Ring({ as: As, onClick, pct, children }) {
  const R = 34;
  const C = 2 * Math.PI * R;
  return (
    <As
